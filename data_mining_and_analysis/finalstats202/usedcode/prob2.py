...
def get_first_day(patient_d):
    return array([patient_a[0]['xvalues'] for patient_a in patient_d.values()])


def k_means_clustering(x, n_clusters):
    #best_k = max(range(2, 11), key=lambda k: np.mean(cross_val_score(KMeans(n_clusters=k), cv=10)))
    kf = KFold(n_splits=10, shuffle=True, random_state=42)
    kmeans = KMeans(n_clusters=n_clusters)
    kmeans.fit(x)
    cluster_labels = kmeans.labels_
    cluster_centers = kmeans.cluster_centers_
    plt.scatter(x[:, 0], x[:, 7],  marker='o')
    print("x", x)
    #In this case x:,0 and X:,7 are the p1 and n1 values
    plt.scatter(kmeans.cluster_centers_[:, 0], kmeans.cluster_centers_[:, 1], marker='X')
    plt.show()
    return cluster_labels, cluster_centers
def dendo_construct(x_a):
    linkage_matrix = hierarchy.linkage(x_a, method='ward')
...
def main():
    ...
    first_x_a = get_first_day(patient_d)
    cluster_labels, cluster_centers = k_means_clustering(first_x_a,3)
    ...
    dendo_construct(get_first_day(patient_d))
