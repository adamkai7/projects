...
patient_visit_dt = dtype([('study','U10'),('country','U10'),('txgroup','U10'),('assesmentid', float64),('patientid', float64),('visitday', int32),('xvalues',float64,(30)),('panss',float64), ('leadstatus','U10')])
...
def desired_data1(patient_d, target_day = 129.5):
    predicted_data_l = []
    for patient_id, patient_a in patient_d.items():
        
        week = (patient_a[-1]['visitday']-patient_a[0]['visitday'])/7
        if week > 18:
            print('more days', patient_id, week)
        day_v = patient_a['visitday']
        day_v = day_v - day_v[0]
        panss_v = patient_a['panss']

        if target_day>day_v[-1] and patient_a.shape[0] > 1 and day_v[-1] != day_v[-2]:
            m = (panss_v[-1]-panss_v[-2])/(day_v[-1]-day_v[-2])
            panss = m*(target_day-day_v[-1]) + panss_v[-1]
        else:
            panss = interp(target_day, day_v, panss_v)

        
        predicted_data_l.append((patient_id, panss))
    return array(predicted_data_l)

def desired_data(patient_d, target_day = 129.5):
    predicted_data_l = []
    for patient_id, patient_a in patient_d.items():
...
def main():
    ...
    #print(da)
    patient_d = find_patients(test_data_a)
    ...
    # plt.hist(control_patientdiff, bins = 10)
    # plt.xlabel('Newest Measurement - Oldest Measurement')
    # plt.ylabel('Frequency')
    # plt.title('Histogram of Differences in Control Group')
    # plt.show()
    ...
    # plt.hist(treatment_patientdiff, bins = 10)
    # plt.xlabel('Newest Measurement - Oldest Measurement')
    # plt.ylabel('Frequency')
    # plt.title('Histogram of Differences in Treatment Group')
    # plt.show()
    ...
    raise SystemExit
    plot_patient_data(control_d)
    plot_patient_data(treatment_d)
    raise SystemExit
