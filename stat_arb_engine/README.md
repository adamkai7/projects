...
There isn’t a single “script that does everything” — the CLI wires the modules together.

- **Main entry / CLI:** [`src/cli.py`](src/cli.py) — run via `stat-arb ...` after install  
- **Cointegration screen:** [`src/screening/cointegration.py`](src/screening/cointegration.py)  
- **Z-score signals:** [`src/signals/zscore.py`](src/signals/zscore.py)  
- **Backtest + costs:** [`src/backtest/engine.py`](src/backtest/engine.py)  
- **Walk-forward / Monte Carlo:** [`src/validation/walkforward.py`](src/validation/walkforward.py)  
- **Data ingest:** [`src/pipeline/ingest.py`](src/pipeline/ingest.py)  
- **Config:** [`configs/default.yaml`](configs/default.yaml)

More technical detail and tabulated results: [`docs/PROJECT_INFO.md`](docs/PROJECT_INFO.md)
